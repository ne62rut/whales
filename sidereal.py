# -*- coding: utf-8 -*-
"""
Created on Mon Feb  1 15:36:04 2016
...
def parseDate ( s ):
    ...
    if  m is None:
        raise SyntaxError(
            "Date does not have pattern YYYY-DD-MM: "
            "'%s'" % s )
    #-- 2 --
...
def parseTime ( s ):
    ...
    if  minuteTail.startswith(':'):
        ...
        if  m is None:
            raise SyntaxError(
                "Expecting minutes: '%s'" % minuteTail )
        else:
    ...
    if  secondTail.startswith(':'):
        ...
        if  m is None:
            raise SyntaxError(
                "Expecting seconds: '%s'" % secondTail)
        else:
...
def parseFixedZone ( s ):
    ...
    if  m is None:
        raise SyntaxError(
            "Expecting zone modifier as %sHHMM: "
            "'%s'" % (s[0], s) )
    else:
        hours  =  int ( rawHHMM[:2] )
