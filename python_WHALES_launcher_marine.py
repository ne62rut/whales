...
def python_whales_launch(filename,mission,saving_directory,Path_dir='..'):
    ...
    J1_filter = np.loadtxt(os.path.join(Path_dir,'cal2/J1_MeanFilterKu'))
    J1_filter_norm = J1_filter / np.mean(J1_filter[11:115])
    J2_filter = np.loadtxt(os.path.join(Path_dir,'cal2/J2_MeanFilterKu'))
    J2_filter_norm = J2_filter / np.mean(J2_filter[11:115])
    J3_filter = np.loadtxt(os.path.join(Path_dir,'cal2/J3_MeanFilterKu'))
    ...
    if mission in ['envisat']:
        ...
        my_path_weights = os.path.join(Path_dir,'weights/weights_n1.mat')
    elif mission in ['jason1']:
        my_path_instr_corr_SWH = os.path.join(Path_dir,'instr_corr/SWHinstrcorr_MLE4_jason1SGDRc.mat')
        my_path_weights = os.path.join(Path_dir,'weights/weights.mat')
        ...
        my_path_instr_corr_SWH = os.path.join(Path_dir,'instr_corr/SWHinstrcorr_WHALES_jason3SGDRd.mat')
        my_path_weights = os.path.join(Path_dir,'weights/weights.mat')
        ...
        my_path_instr_corr_SWH = ''
        my_path_weights = os.path.join(Path_dir,'weights/weights_cs2_lrm.mat')
    ...
    for index_waveforms_row in np.arange(378,380):#np.arange(0, np.shape(S_time)[0], 1):
